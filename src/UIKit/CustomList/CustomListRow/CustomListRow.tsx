...
function CustomListRow<ItemType = any>(props: ListRowProps<ItemType>) {
	...
	const getRowClassname = (): string => {
		...
		if (getDetailsLayout && isShowDetails || isOpen) return "custom-list-row-uikit custom-list-row-uikit_open"
		// Скрыта детальная информация, можно развернуть
		if (getDetailsLayout || isClickable) return "custom-list-row-uikit custom-list-row-uikit_openable"
		// Нельзя развернуть детальную информацию
		return "custom-list-row-uikit"
	}
